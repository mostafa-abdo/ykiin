...
class CausesController extends Controller
{
    ...
    public function approve_donation_payment(Request $request, $id)
    {
        ...
        $event_details = Cause::find($payment_logs->cause_id);
      
        if($payment_logs->added_in_raised_amount == 0){
           $raised__amount = floatval($payment_logs->amount);
            $event_details->raised += $raised__amount;
            $event_details->save();
        }
    ...
    public function settings()
    {
        ...
        return view(self::BASE_PATH . 'settings', compact('pages'));
    }
