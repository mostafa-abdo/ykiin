...
//===================== USER CAMPAIGNS CODE =====================//
    ...
    public function campaign_log_withdraw()
    {
        ...
        return view(self::BASE_PATH . 'campaigns.campaign-log-withdraw')
            ->with([
                'causes' => $causes,
    ...
    public function campaign_withdraw_submit(Request $request)
    {
        ...
        $fields = [];
        foreach($request->gateway_field ?? [] as $key => $value){
        ...
        $withdraw_id = DonationWithdraw::create([
            ...
            "gateway_id" => (int) $request->gateway_name,
    ...
    public function support_ticket_message(Request $request){
        $this->validate($request,[
            ...
            'file' => 'nullable|mimes:zip,png,jpg,jpeg,webp,gif',
        ]);
        ...
        if ($request->hasFile('file')){
            ...
            if (in_array($file_extension, ['jpg', 'jpeg', 'png', 'gif', 'webp'])) {
                $processed_image = Image::make($uploaded_file)->resize(740, null, function ($constraint) {
    ...
    public function tax_information_update(Request $request){
            $request->validate([
                ...
                'nid_image' => Rule::requiredIf(function () use ($request) {
                    return empty($request->driving_license_image) && empty($request->passport_image);
                }),
                'driving_license_image' => Rule::requiredIf(function () use ($request) {
                    return empty($request->nid_image) && empty($request->passport_image);
                }),
                'passport_image' => Rule::requiredIf(function () use ($request) {
                    return empty($request->nid_image) && empty($request->driving_license_image);
                }),
            ]);
