...
class EventPaymentLogsController extends Controller
{
    ...
    public function booking_payment_form(Request $request){
        ...
        if (!empty($event_info->cost) && $event_info->cost == 0){
            $event_payment_details->status = 'complete';
            $event_payment_details->save();
            $event_details->status = 'complete';
            $event_details->payment_status = 'complete';
            $event_details->save();
        }
        ...
        elseif ($request->selected_payment_gateway === 'senangpay') {
            try{

                $senangpay = PaymentGatewayRequestHelper::senangpay();

                $response = $senangpay->charge_customer(
                    $this->common_charge_customer_data($event_details,$event_payment_details,route('frontend.event.senangpay.ipn'))
                );
                return $response;
            }
            catch(\Exception $e){
        ...
        elseif ($request->selected_payment_gateway === 'iyzipay') {
            try{
                ...
                $response = $iyzipay->charge_customer(
                    $this->common_charge_customer_data($event_details,$event_payment_details,route('frontend.event.iyzipay.ipn'))
                );
                return $response;
            }
            catch(\Exception $e){
        ...
        elseif ($request->selected_payment_gateway === 'awdpay') {
            try{
                ...
                $response = $awdpay->charge_customer(
                    $this->common_charge_customer_data($event_details,$event_payment_details,route('frontend.event.awdpay.ipn'))
                );
                return $response;
            }
            catch(\Exception $e){
