...
class FrontendEventController extends Controller
{
    ...
    public function events_single($slug)
    {
        ...
        $qr_code = QrCode::size(150)->generate(route('frontend.events.single', $event->slug));
    ...
    public function downloadQrCode($slug)
    {
        ...
        $qrCode = QrCode::format('png')
            ...
            ->size(150)
            ->style('square')
            ->backgroundColor(255, 255, 255) // Set background color to white (RGB values)
            ->generate(route('frontend.events.single', $slug));
        // Return the QR code
        return Response::make($qrCode, 200, [
            'Content-Type' => 'image/png',
            'Content-Disposition' => 'attachment; filename="event-qrcode.png"',
        ]);
    }
