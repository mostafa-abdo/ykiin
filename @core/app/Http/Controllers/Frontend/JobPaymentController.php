...
class JobPaymentController extends Controller
{
        ...
        if (!empty($jobs_details->application_fee_status) && $jobs_details->application_fee > 0){
            ...
            if($job_applicant_details->payment_gateway === 'paypal'){
                ...
                $redirect_url = $paystack->charge_customer(
                    $this->common_charge_customer_data($job_applicant_details,$jobs_details,route('frontend.job.paystack.ipn'),'job')
                );
                return $redirect_url;
            ...
            elseif ($job_applicant_details->payment_gateway === 'senangpay') {
                try{

                    $senangpay = PaymentGatewayRequestHelper::senangpay();

                    $response = $senangpay->charge_customer(
                        $this->common_charge_customer_data($job_applicant_details,$jobs_details,route('frontend.event.senangpay.ipn'))
                    );
                    return $response;
                }
                catch(\Exception $e){
            ...
            elseif ($job_applicant_details->payment_gateway === 'iyzipay') {
                try{

                    $iyzipay = PaymentGatewayRequestHelper::iyzipay();

                    $response = $iyzipay->charge_customer(
                        $this->common_charge_customer_data($job_applicant_details,$jobs_details,route('frontend.event.iyzipay.ipn'))
                    );
                    return $response;
                }
                catch(\Exception $e){
            ...
            elseif ($job_applicant_details->payment_gateway === 'awdpay') {
                try{

                    $awdpay = PaymentGatewayRequestHelper::awdpay();

                    $response = $awdpay->charge_customer(
                        $this->common_charge_customer_data($job_applicant_details,$jobs_details,route('frontend.event.awdpay.ipn'))
                    );
                    return $response;
                }
                catch(\Exception $e){
    ...
    {
        $paystackpay = PaymentGatewayRequestHelper::paystack();
        $payment_data = $paystackpay->ipn_response();
        return $this->common_ipn_data($payment_data);
    }
