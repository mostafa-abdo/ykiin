...
class UserController extends Controller
{
    public function login(Request $request)
    {
        ...
        $user = User::select('id', 'email', 'password','deactivate')->where('email', $request->email)->first();

        if(!is_null($user) && $user->deactivate === 1){
            return response()->json([
    ...
    public function register(Request $request)
    {
        $validate = Validator::make($request->all(),[
            ...
            'password' => 'required|min:6|max:191|regex:/^.*(?=.{3,})(?=.*[a-zA-Z])(?=.*[0-9])(?=.*[\d\x])(?=.*[!$#%]).*$/',
            'country_id' => 'required',
            'state' => 'nullable',
        ]);
        if ($validate->fails()){
        ...
        if(!is_null($userInfo) && $userInfo->deactivate === 1){
            return response()->json([
                'message' => __('Your account is deactivated')
            ])->setStatusCode(422);
        }
