...
class AttendanceBookingDatabaseUpdate
{
    ...
    public function handle(AttendanceBooking $event)
    {
        ...
        $payment_log =  EventPaymentLogs::where('attendance_id',$data['attendance_id'])->first();

        //update attendance status
        $order_details = EventAttendance::findOrFail($data['attendance_id']);
        
        $order_details->payment_status = $payment_log->package_gateway === 'manual_payment' ? 'pending' : 'complete';
