...
HTML;
    ...
    public static function paymentInfoColumn($row){
        ...
        $output .= '<li><strong>'.__('Type').': </strong> '. (!is_null($row->recuring_token) ? 'Monthly' : 'One Time') .'</li>';
        ...
        $output .= '<li><strong>'.__('Transaction Fee').': </strong> '.amount_with_currency_symbol($row->transaction_fee).'</li>';
    ...
    public static function withdrawInfoColumn($row){
        ...
        $output .= '<li><strong>'.__('Payment Gateway').': </strong> '
            .ucwords(str_replace('_',' ',
                !empty($row->payment_gateway) ? $row->payment_gateway : ($row?->gateway_name?->name ?? '')))
            .'</li>';
