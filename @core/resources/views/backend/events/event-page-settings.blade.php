                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <form action="{{route('admin.events.page.settings')}}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                <label for="event_terms_and_conditions">{{__('Terms & Conditions')}}</label>
                                <select class="form-control" name="event_terms_and_conditions">
                                    <option value="">{{ __('Select Terms & Conditions Pages to show') }}</option>
                                    @foreach($pages as $page)
                                        <option value="{{ $page->id }}" @if (get_static_option('event_terms_and_conditions') == $page->id)  selected @endif >{{ __($page->title) }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                <label for="event_privacy_policy">{{__('Privacy Policy')}}</label>
                                <select class="form-control" name="event_privacy_policy">
                                    <option value="">{{ __('Select Privacy Policy Pages to show') }}</option>
                                    @foreach($pages as $page)
                                        <option value="{{ $page->id }}" @if (get_static_option('event_privacy_policy') == $page->id)  selected @endif >{{ __($page->title) }}</option>
                                    @endforeach
                                </select>
                                <span class="info-text">{{__('you can create new pages if you do not have.')}}</span>
                            </div>
                            ...
                            <div class="form-group">
                                <label for="event_page_qr_code_title">{{__('Qr Code Title')}}</label>
                                <input type="text" class="form-control" name="event_page_qr_code_title" value="{{get_static_option('event_page_qr_code_title')}}">
                            </div>

                            <div class="form-group">
                                <label for="navbar_button">{{__('Show/Hide Event QR code')}}</label>
                                <label class="switch">
                                    <input type="checkbox" name="event_page_qr_code_show_hide"
                                           @if(!empty(get_static_option('event_page_qr_code_show_hide'))) checked @endif >
                                    <span class="slider"></span>
                                </label>
                            </div>
